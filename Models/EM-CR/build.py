...
def build_model(
    ...
    ----------
    path: str
        Model directory (Models/EM-CR/). Supplied by Foresee.load_model.
    energy: str
    ...
    model = Model("CR", path=path)

    vector_meson_br = (
        "2 * {ratio} * coupling**2 *({m}**4.)/(4.*np.pi/137.) "
        "* ((1+2*(mass/{m})**2)*np.sqrt(1-4*(mass/{m})**2) )"
    ...
    dalitz_br = (
        "2.*{ratio} * coupling**2 *(q**2.)/16./(np.pi**2.) "
        "* (1-q**2/{m}**2)**3 * (1-4*mass**2/q**2)**0.5 "
        "* (2.-(1-4*mass**2/q**2)*np.sin(th)**2)"
    )
    threebody = [
        ("111", "0.99",  "0.135"),
        ("221", "0.39",  "0.547"),
    ...
    model.set_dsigma_drecoil_1d(
        dsigma_der="1./137 * (2*0.000511 - recoil*mass**2/energy**2)",
