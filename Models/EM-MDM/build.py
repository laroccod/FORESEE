...
def build_model(
    ...
    ----------
    path: str
        Model directory (Models/EM-MDM/). Supplied by Foresee.load_model.
    energy: str
    ...
    model = Model("MDM", path=path)

    vector_meson_br = (
        "2 * {ratio} * coupling**2 *(296.342)**2. *({m}**2.)/(8.*np.pi/137.) "
        "* ((1+8*(mass/{m})**2)*np.sqrt(1-4*(mass/{m})**2) )"
    ...
    dalitz_br = (
        ...
        "* (8.*mass**2./q**2.+(1-4*mass**2/q**2)*np.sin(th)**2)"
    )
    threebody = [
        ("111", "0.99",  "0.135"),
        ("221", "0.39",  "0.547"),
